...
def classify_and_summarize(input_text, cls_model, tokenizer_cls, label_encoder, model_summ, tokenizer_summ, device,current_request_timestamp):
    # ... (Your existing implementation)
    results = []
    ...
    for url in input_text:
        ...
        insert_question_and_answer(url,answer, current_request_timestamp)
    ...
    if request.method == 'POST':
        ...
        if url_input.startswith("http"):
            ...
            totalresult = classify_and_summarize(
                url_input, cls_model, tokenizer_cls, label_encoder, model_summ, tokenizer_summ, device,current_request_timestamp
            )
